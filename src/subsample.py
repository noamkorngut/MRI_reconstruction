...
class EquispacedMaskFunc(MaskFunc):
    ...
    def __call__(
        ...
        with temp_seed(self.rng, seed):
            ...
            accel_samples = np.arange(0, num_cols - 1, adjusted_accel)
...
if __name__ == '__main__':
    mask_eq = create_mask_for_mask_type('equispaced', [0.04], [8])
    mask_rand = create_mask_for_mask_type('random', [0.04], [8])
    mask_eq = mask_eq((30,370,300), None)
    mask_rand = mask_rand((30,370,300), None)

    plt.figure()
    plt.imshow(mask_eq.expand(1,370,300).numpy()[0,:,:], cmap= 'gray'), plt.title('equispaced')
    plt.show
    plt.figure()
    plt.imshow(mask_rand.expand(1,370,300).numpy()[0,:,:], cmap= 'gray'), plt.title('random')
    plt.show
