...
def pil2tensor(im):  # in: [PIL Image with 3 channels]. out: [B=1, C=3, H, W] (0, 1)
    ...
    im = transform(im.squeeze(0))
    print(type(im), 'hhhh')
    #print(im.shape, 'vvvvv')
    print(torch.Tensor((np.float32(im)).transpose(2, 0 ,1)).unsqueeze(0).shape)
    return torch.Tensor((np.float32(im)).transpose(2, 0 ,1)).unsqueeze(0)

def get_exp_name ():

    now = datetime.now() # current date and time
    exp_name = 'KIKI_models' + now.strftime("%Y_%m_%d_%H_%M_%S")
...
def train(args, load_model= False, model_path=None):
    ...
    out_folder = f'./R{args.accelerations}_lamb{args.lambda_}_bs{args.batch_size}_channels{args.fm}_lr{args.lr}_layers{args.k}/'
    ...
    #model = UNet(2,1)#UNet(args.in_ch, args.out_ch)
    ...
    model = UNet(
        in_ch = args.in_ch, #1
        out_ch = args.out_ch, #1
        chans = args.channels, #args.chans, #32
    ...
    for epoch in range(pre_epoch, args.epochs):
        ...
        for i, train_data in enumerate(train_loader,0):
            ...
            print(train_kspaces_us.shape, 'input shape')
            rec =  model(train_kspaces_us) ##todo model(train_kspaces_us, train_mask)
            print(rec.shape, 'rec')
            train_imgs_rec = ifft2c(rec.permute(0,2,3,1))#rec['recon_image']
            ...
            loss_train = criterion((train_imgs_rec), (train_imgs_fs.permute(0,2,3,1)))# + args.lambda_* criterion(train_kspace_rec*(1-train_mask), train_kspaces_fs*(1-train_mask))
        ...
        with torch.no_grad():
            ...
            for i, valid_data in enumerate(val_loader):
                ...
                rec = model(valid_kspaces_us) #todo model(valid_kspaces_us, valid_mask)
                import fftc
                valid_imgs_rec = ifft2c(rec.permute(0,2,3,1))#['recon_image']
                #valid_imgs_rec = valid_imgs_rec.permute(0,2,3,1)
                #valid_kspace_rec = rec['recon_ksapce']

                loss_valid = criterion(valid_imgs_rec, valid_imgs_fs.permute(0,2,3,1)) #+ args.lambda_* criterion(valid_kspace_rec*(1-valid_mask), valid_kspaces_fs*(1-valid_mask))
